...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      default:
        if (!action) return await handleHealthCheck();
...
async function handleKeys(pattern: string | null) {
  ...
  try {
    const keys = await redisCache.keys(searchPattern);
...
async function handleStats() {
  try {
    const [dbsize, info, memory] = await Promise.all([
      redisCache.dbsize(),
      redisCache.info('keyspace'),
      redisCache.info('memory'),
    ]);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'increment':
        ...
        const newValue = await redisCache.increment(key);
        return NextResponse.json({
          success: true,
          key,
          newValue,
      ...
      case 'decrement':
        ...
        const decrValue = await redisCache.decrement(key);
        return NextResponse.json({
          success: true,
          key,
          newValue: decrValue,
