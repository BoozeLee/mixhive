...
export function BuzzDetail() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <button
        onClick={() => navigate(-1)}
        style={{ background: 'none', border: 'none', color: colors.text.muted, cursor: 'pointer', fontSize: fontSize.base, marginBottom: space[8], display: 'flex', alignItems: 'center', gap: space[4] }}
      >
        ← Back
      </button>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 40, color: colors.text.dim }}>Buzz not found.</div>
      ) : (
        <>
          {/* Parent buzz */}
          ...
          {/* Reply composer */}
          {user ? (
            <div style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: radius.xl, padding: `${space[7]}px ${space[9]}px`, margin: `${space[7]}px 0` }}>
              <div style={{ display: 'flex', gap: space[7] }}>
                ...
                <div style={{ flex: 1 }}>
                  <textarea
                    ...
                    style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', resize: 'none', color: colors.text.primary, fontSize: fontSize.base, lineHeight: 1.5, fontFamily: 'inherit', boxSizing: 'border-box' }}
                  />
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: space[6], marginTop: space[4] }}>
                    <span style={{ fontSize: fontSize.sm, color: remaining < 60 ? (remaining <= 0 ? colors.danger : colors.warning) : colors.text.dim }}>
                      {remaining < 60 ? remaining : ''}
                    </span>
                    <button
                      onClick={handleReply}
                      disabled={!canReply}
                      style={{ padding: `${space[3]}px ${space[8]}px`, background: canReply ? colors.accent : colors.surfaceMuted, color: canReply ? '#0a0a0a' : colors.text.faint, border: 'none', borderRadius: radius.pill, fontWeight: fontWeight.bold, fontSize: fontSize.sm, cursor: canReply ? 'pointer' : 'default' }}
                    >
                      {replyBusy ? '…' : 'Reply'}
                    </button>
                  </div>
            ...
            <div style={{ textAlign: 'center', padding: `${space[7]}px 0`, color: colors.text.dim, fontSize: fontSize.base }}>
              <button onClick={() => navigate('/login')} style={{ background: 'none', border: 'none', color: colors.accent, cursor: 'pointer', fontWeight: fontWeight.semibold, fontSize: fontSize.base }}>Sign in</button>
              {' '}to reply
            </div>
          )}
          ...
          {replies.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: space[4] }}>
              <p style={{ margin: `0 0 ${space[4]}px`, fontSize: fontSize.sm, color: colors.text.dim, fontWeight: fontWeight.semibold }}>
