...
const STYLE_SEEDS: Record<string, string> = {
  'cyber-hive': 'dark cyberpunk DJ, glowing gold hexagon patterns, honeycomb, neon, ultra high detail',
...
export async function POST(req: NextRequest) {
  ...
  if (!hfKey) {
    return NextResponse.json({ error: 'Pro AI art not yet configured on this instance' }, { status: 503 })
  }
  ...
  if (!ctx.isPro) {
    return NextResponse.json(
      { error: 'pro_required', message: 'Upgrade to MixHive Pro to use the built-in AI art generator.' },
      { status: 403 },
    )
  }
