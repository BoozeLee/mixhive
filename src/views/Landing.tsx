...
function HiveMap() {
  const nodes = [
    [14, 55], [26, 68], [38, 46], [50, 75], [61, 50], [74, 64], [86, 42], [48, 30],
  ]
  return (
    <HiveCard tone="panel" style={{ minHeight: 230, overflow: 'hidden' }}>
      ...
      <svg
        ...
        style={{ width: '100%', height: 190, marginTop: 12, filter: 'drop-shadow(0 0 10px rgba(246,196,0,0.4))' }}
      >
        <defs>
        ...
        <path d="M5 52 C18 30, 33 50, 45 28 S77 22, 96 48" fill="none" stroke="rgba(246,196,0,.18)" strokeWidth="7" />
        <rect x="4" y="8" width="92" height="54" fill="url(#dots)" opacity=".75" />
        {nodes.slice(1).map((node, index) => (
          <line key={`${node[0]}-${node[1]}`} x1={nodes[index][0]} y1={nodes[index][1]} x2={node[0]} y2={node[1]} stroke="rgba(246,196,0,.55)" strokeWidth=".4" />
        ))}
        {nodes.map(node => <circle key={`${node[0]}-${node[1]}`} cx={node[0]} cy={node[1]} r="2.1" fill="#f6c400" />)}
      </svg>
    </HiveCard>
  )
}
    ...
    <HiveCard tone="panel" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      {mixes.length === 0 ? (
        ...
        mixes.slice(0, 3).map((mix, i) => (
          <article key={mix.id} style={{
            padding: '16px 22px',
            borderBottom: i === Math.min(mixes.length, 3) - 1 ? 'none' : '1px solid var(--hive-line-soft, rgba(246,196,0,0.12))',
            display: 'flex',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div
                ...
                style={{
                  ...
                  height: 38,
                  borderRadius: 10,
                  background: 'linear-gradient(135deg, var(--hive-surfaceHi, #1c1812), var(--hive-gold-deep, #c79100))',
                  display: 'flex',
                  alignItems: 'center',
...
export function Landing() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setStats(s)
      setFeed(f.data ?? [])
    }).catch(() => { /* silent — fallback to zeros */ })
      ...
      <main style={{
        ...
        <section style={{
          ...
          <div style={{
            ...
            {/* Left: wordmark + CTA */}
            <HiveCard tone="panel" style={{ padding: 36, minHeight: 380, position: 'relative', overflow: 'hidden' }}>
              <p style={{
                color: 'var(--hive-gold, #f6c400)',
                fontFamily: 'var(--font-ui)',
                fontWeight: 800,
              ...
              <h1 style={{
                ...
                The Hive<br />Is Your<br />
                <span style={{ color: 'var(--hive-gold, #f6c400)', textShadow: '0 0 24px rgba(246,196,0,0.45)' }}>
                  Network
                </span>
              </h1>
