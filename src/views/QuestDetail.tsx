...
export function QuestDetail() {
  ...
  if (loading) {
    return <div style={{ padding: 60, textAlign: 'center', color: colors.text.muted }}>Loading quest…</div>;
  }

  if (!questData) {
    return <EmptyState title="Quest not found" body="This quest doesn't exist or you don't have access to it." />;
  }
  ...
  if (!id) {
    return <EmptyState title="Quest not found" body="The quest you're looking for doesn't exist or you don't have access." />;
  }
  ...
  return (
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '32px 16px' }}>
      ...
      <HiveCard tone="glow" style={{ marginBottom: space[8] }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: fontSize.sm }}>{milestones.filter(m => m.status === 'completed').length} / {milestones.length} milestones</div>
          </div>
        </div>
      ...
      <HiveCard style={{ marginBottom: space[8] }}>
        <div style={{ fontSize: fontSize.sm, color: colors.text.muted, marginBottom: space[2] }}>NARRATIVE SUMMARY</div>
        <p style={{ margin: 0, lineHeight: 1.5 }}>
          {quest.description || "You're building real traction in the Brussels underground. Recent performances and targeted submissions are creating momentum. Focus on consistent local presence and one high-signal collaboration."}
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: space[4] }}>
          {milestones.map((m, index) => (
            <div key={m.id} style={{
              ...
              <div style={{ fontSize: 22, width: 32, textAlign: 'center', opacity: m.status === 'completed' ? 1 : 0.6 }}>
        ...
        <div style={{ display: 'grid', gap: space[4], gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' }}>
          {actions.map(action => (
            <HiveCard key={action.id} interactive tone="panel">
              <div style={{ fontWeight: fontWeight.semibold, marginBottom: space[2] }}>{action.title}</div>
              <div style={{ fontSize: fontSize.sm, color: colors.text.secondary, marginBottom: space[4] }}>{action.rationale}</div>
              <div style={{ display: 'flex', gap: space[3], alignItems: 'center' }}>
                <HiveButton size="sm" variant="primary">Take Action</HiveButton>
      ...
      <HiveCard>
        ...
        <div style={{ fontSize: fontSize.sm, color: colors.text.secondary }}>
          {log.map((entry, i) => (
            <div key={i} style={{ marginBottom: space[3], paddingLeft: space[2], borderLeft: `3px solid ${colors.accentMuted}` }}>
      ...
      <div style={{ marginTop: space[10], textAlign: 'center', fontSize: fontSize.sm, color: colors.text.faint }}>
        This is a prototype view. Real data + agent proposals will appear here once the backend is wired.
      </div>
    </div>
  );
}
