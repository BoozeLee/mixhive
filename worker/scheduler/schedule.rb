...
JOBS = {
  ...
  '/api/cron/push-sender'              => Integer(ENV.fetch('IV_PUSH', 3600)),         # hourly
  ...
  '/api/cleanup'                       => Integer(ENV.fetch('IV_CLEANUP', 86_400)),     # daily (stub)
  '/api/analytics/daily'               => Integer(ENV.fetch('IV_ANALYTICS', 86_400))    # daily (stub)
}.freeze
