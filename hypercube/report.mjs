...
const json = {
  ...
  defects: ranked.map(c => ({
    severity: SEV[severity(c)],
    target: c.target,
    env: c.env,
    dims: c.dims,
    defect: c.defect,
...
L.push(
  `**Verdict: ${verdict}**  ·  executed ${cells.length} cells → ✅ ${pass.length} pass · ❌ ${fail.length} fail · ⛔ ${blocked.length} blocked(expected-gate)`,
  ''
);
L.push(
  `Model: t=2 pairwise covering array, **overall ρ = ${Number(cov.overallRho).toFixed(4)}** across ${cov.rows} planned cells (see \`generated/coverage.json\`).`,
  ''
);
...
if (!blocked.length) L.push('_None._', '');
  ...
  for (const [reason, ts] of Object.entries(byReason)) {
    ...
    L.push(
      `- **${reason}** (${ts.length} cells, ${uniq.length} targets): ${uniq.slice(0, 8).join(', ')}${uniq.length > 8 ? '…' : ''}`
    );
  }
  L.push('');
}
...
for (const cl of classes) {
  ...
  L.push(
    `| ${cl} | ${cc.filter(c => c.verdict === 'pass').length} | ${cc.filter(c => c.verdict === 'fail').length} | ${cc.filter(c => c.verdict === 'blocked').length} |`
  );
}
L.push('');
...
console.log(
  `Report: ${verdict} — ${pass.length} pass / ${fail.length} fail / ${blocked.length} blocked (${cells.length} executed). → hypercube/results/report.md`
);
