...
const emptyRole = (): RoleForm => ({
  role_type: '', title: '', skill_tags: '', experience_level: 'any',
  is_paid: false, compensation_notes: '',
});
...
export function NewCollabQuest() {
  ...
  const updateGoal = (i: number, v: string) =>
    setForm(f => { const g = [...f.goals]; g[i] = v; return { ...f, goals: g }; });
    ...
    try {
      ...
      const res = await fetch('/api/collab-quests', {
        ...
        body: JSON.stringify({
          ...
          discipline_tags: form.discipline_tags.split(',').map(t => t.trim()).filter(Boolean),
          ...
          roles: validRoles.map(r => ({
            ...
            title: r.title.trim(),
            skill_tags: r.skill_tags.split(',').map(t => t.trim()).filter(Boolean),
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <label style={labelStyle}>
              Quest title *
              <input value={form.title} onChange={e => updateForm('title', e.target.value)} placeholder='e.g. "Build visual identity for my techno alias"' style={inputStyle} />
            </label>
            <label style={labelStyle}>
              Narrative (RPG-style context)
              <textarea value={form.narrative} onChange={e => updateForm('narrative', e.target.value)} placeholder="Describe the project in your own words..." rows={3} style={{ ...inputStyle, resize: 'vertical', fontFamily: 'inherit' }} />
            </label>
            <div>
              <p style={{ color: '#aaa', fontSize: 13, marginBottom: 8 }}>Goals * (what will be delivered)</p>
              {form.goals.map((g, i) => (
                <input
                  key={i}
                  value={g}
        ...
        <section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <label style={labelStyle}>
              Discipline tags
              <input value={form.discipline_tags} onChange={e => updateForm('discipline_tags', e.target.value)} placeholder="visual_artist, dj" style={inputStyle} />
            </label>
            <label style={labelStyle}>
              Region (optional)
              <input value={form.region} onChange={e => updateForm('region', e.target.value)} placeholder="Brussels, Remote, Europe" style={inputStyle} />
            </label>
            <label style={labelStyle}>
              Timeline (days)
              <input type="number" value={form.timeline_days} onChange={e => updateForm('timeline_days', e.target.value)} placeholder="30" min="1" style={inputStyle} />
            </label>
            <label style={labelStyle}>
              XP reward
              <input type="number" value={form.xp_reward} onChange={e => updateForm('xp_reward', e.target.value)} min="0" max="2000" style={inputStyle} />
            </label>
          </div>
        </section>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {roles.map((role, i) => (
              <div key={i} style={{ background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: 10, padding: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <label style={labelStyle}>
                    ...
                    <select value={role.role_type} onChange={e => updateRole(i, 'role_type', e.target.value)} style={inputStyle}>
                      ...
                      {ROLE_TYPES.map(rt => <option key={rt} value={rt}>{rt.replace('_', ' ')}</option>)}
                    </select>
                  </label>
                  <label style={labelStyle}>
                    Label *
                    <input value={role.title} onChange={e => updateRole(i, 'title', e.target.value)} placeholder="Lead Animator" style={inputStyle} />
                  </label>
                  <label style={labelStyle}>
                    Skills (comma-sep)
                    <input value={role.skill_tags} onChange={e => updateRole(i, 'skill_tags', e.target.value)} placeholder="after_effects, 3d" style={inputStyle} />
                  </label>
                  <label style={labelStyle}>
                    Experience level
                    <select value={role.experience_level} onChange={e => updateRole(i, 'experience_level', e.target.value)} style={inputStyle}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#aaa', fontSize: 13, marginTop: 10, cursor: 'pointer' }}>
                  <input type="checkbox" checked={role.is_paid} onChange={e => updateRole(i, 'is_paid', e.target.checked)} />
                  Paid role
                </label>
                {role.is_paid && (
                  <input
        ...
        <button
          ...
          disabled={submitting}
          style={{ ...primaryBtnStyle, opacity: submitting ? 0.6 : 1, alignSelf: 'flex-end', padding: '12px 32px' }}
        >
