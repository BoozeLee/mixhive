...
export function Settings() {
  ...
  return (
    <div className="container" style={{ maxWidth: 540, margin: '0 auto', padding: '24px 16px 96px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <fieldset style={{ border: 'none', padding: 0, margin: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 8 }}>
            {['twitter', 'instagram', 'soundcloud', 'youtube', 'spotify', 'website'].map(platform => (
              <div key={platform} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 16, width: 20, textAlign: 'center' }}>{getPlatformIcon(platform)}</span>
                <Input
                  type="text"
          ...
          <p style={{ color: '#666', fontSize: 11, marginTop: 4 }}>Add links to your social profiles or website</p>
        </fieldset>
      ...
      <div id="ai" style={{ marginTop: space[11], borderTop: `1px solid ${colors.border}`, paddingTop: space[10] }}>
        ...
        {/* Pro badge */}
        {profile?.is_pro && (
          <div style={{ display: 'flex', alignItems: 'center', gap: space[4], background: `linear-gradient(135deg, ${colors.surface}, ${colors.accentFaint})`, border: `1px solid ${colors.accentMuted}`, borderRadius: radius.lg, padding: `${space[5]}px ${space[7]}px`, marginBottom: space[7] }}>
            <span style={{ fontSize: 22 }}>🐝</span>
            <div>
              <div style={{ fontWeight: fontWeight.bold, fontSize: fontSize.base, color: colors.accent }}>MixHive Pro — AI Art Active</div>
              <div style={{ fontSize: fontSize.xs, color: colors.text.muted }}>Stable Diffusion XL — no key needed</div>
            </div>
          </div>
        )}
        ...
        {!profile?.is_admin && (
          <div style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: `${space[7]}px` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: space[5] }}>
              <span style={{ fontWeight: fontWeight.semibold, fontSize: fontSize.base, color: colors.text.primary }}>OpenAI API key</span>
              {aiKeyStatus === 'loading' && <span style={{ fontSize: fontSize.xs, color: colors.text.dim }}>checking…</span>}
              {aiKeyStatus === 'saved'   && <span style={{ fontSize: fontSize.xs, color: colors.success, fontWeight: fontWeight.semibold }}>● Connected</span>}
              {aiKeyStatus === 'none'    && <span style={{ fontSize: fontSize.xs, color: colors.text.faint }}>Not connected</span>}
            </div>

            {aiKeyStatus === 'saved' ? (
              <div style={{ display: 'flex', gap: space[4] }}>
                <div style={{ flex: 1, background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: radius.md, padding: `${space[4]}px ${space[5]}px`, fontSize: fontSize.sm, color: colors.text.muted, fontFamily: 'monospace' }}>
                  sk-••••••••••••••••••••••
                </div>
                <button onClick={handleRemoveAiKey} disabled={aiKeySaving} style={{ padding: `${space[4]}px ${space[7]}px`, borderRadius: radius.md, border: `1px solid ${colors.danger}`, background: 'transparent', color: colors.danger, fontSize: fontSize.sm, cursor: 'pointer' }}>
                  Remove
                </button>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: space[4] }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <button
                    ...
                    style={{ position: 'absolute', right: space[4], top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: colors.text.dim, fontSize: fontSize.sm }}
                ...
                <button
                  ...
                  style={{ padding: `${space[4]}px`, borderRadius: radius.md, background: colors.accent, color: '#0a0a0a', border: 'none', fontWeight: fontWeight.semibold, fontSize: fontSize.sm, cursor: 'pointer', opacity: aiKeySaving || !aiKeyInput.trim() ? 0.5 : 1 }}
                >
            ...
            {aiKeyMsg && (
              <p style={{ margin: `${space[4]}px 0 0`, fontSize: fontSize.xs, color: aiKeyMsg.includes('saved') || aiKeyMsg.includes('unlocked') ? colors.success : colors.danger }}>
                {aiKeyMsg}
              </p>
            )}
          </div>
        ...
        {!profile?.is_pro && !profile?.is_admin && (
          <div style={{ marginTop: space[7], background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: `${space[7]}px`, display: 'flex', alignItems: 'center', gap: space[7] }}>
            <span style={{ fontSize: 28, flexShrink: 0 }}>🐝</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: fontWeight.semibold, fontSize: fontSize.base, color: colors.text.primary, marginBottom: space[2] }}>MixHive Pro — Built-in AI art</div>
              <div style={{ fontSize: fontSize.xs, color: colors.text.muted, lineHeight: 1.5 }}>No key needed. Generate Stable Diffusion XL avatars directly on MixHive, plus priority support and more storage.</div>
            </div>
            <a
              href="mailto:admin@mixhive.app?subject=MixHive Pro"
              style={{ flexShrink: 0, padding: `${space[4]}px ${space[7]}px`, background: colors.accent, color: '#0a0a0a', borderRadius: radius.pill, fontWeight: fontWeight.bold, fontSize: fontSize.sm, textDecoration: 'none', whiteSpace: 'nowrap' }}
            >
              Get Pro
            </a>
          </div>
