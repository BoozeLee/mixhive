...
export default function AdminAgentsPage() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'monospace', background: '#0a0a0a', color: '#e0e0e0' }}>
      {/* Sidebar */}
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
        {!selected ? (
          ...
          <>
            ...
            <p style={{ color: '#888', margin: '0 0 16px', fontSize: 13 }}>{selected.description}</p>
            ...
            {status && (
              <div style={{
                ...
                color: status.startsWith('Error') ? '#f44336' : '#4caf50', fontSize: 13,
              }}>{status}</div>
            )}

            {/* Version history */}
            <h3 style={{ color: '#ccc', borderBottom: '1px solid #222', paddingBottom: 8 }}>Version History</h3>
              ...
              {versions.map(v => {
                ...
                return (
                  <div key={v.id} style={{
                    ...
                    background: '#111', border: `1px solid ${isLive ? '#ffd700' : isRolledBack ? '#333' : '#222'}`,
                    opacity: isRolledBack ? 0.5 : 1,
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontWeight: 700, color: isLive ? '#ffd700' : '#aaa' }}>v{v.version}</span>
                      {isLive && <span style={{ background: '#ffd700', color: '#000', padding: '1px 6px', borderRadius: 3, fontSize: 11, fontWeight: 700 }}>LIVE</span>}
                      {isRolledBack && <span style={{ background: '#333', color: '#888', padding: '1px 6px', borderRadius: 3, fontSize: 11 }}>ROLLED BACK</span>}
                      {v.promoted_at && !isRolledBack && <span style={{ color: '#4caf50', fontSize: 11 }}>promoted</span>}
                      <span style={{ color: '#555', fontSize: 11, marginLeft: 'auto' }}>{v.author ?? 'unknown'} · {new Date(v.created_at).toLocaleDateString()}</span>
                    </div>
                    {v.notes && <div style={{ color: '#aaa', fontSize: 12, marginTop: 4 }}>{v.notes}</div>}
                    {!isLive && !isRolledBack && (
                      <button
                        onClick={() => handlePromote(v.version)}
                        style={{ marginTop: 8, padding: '4px 10px', background: '#1a3a00', border: '1px solid #4caf50', color: '#4caf50', borderRadius: 3, cursor: 'pointer', fontSize: 12 }}
                      >
                        Promote to live
                      </button>
                    )}
            ...
            {selected.lua_script_version > 1 && (
              <button
                ...
                style={{ padding: '6px 14px', background: '#2d0000', border: '1px solid #f44336', color: '#f44336', borderRadius: 4, cursor: 'pointer', fontSize: 13, marginBottom: 24 }}
              >
            ...
            {/* Create new version */}
            <h3 style={{ color: '#ccc', borderBottom: '1px solid #222', paddingBottom: 8 }}>Create Draft Version</h3>
            <input
              type="text"
              placeholder="Version notes (optional)"
              value={newNotes}
              onChange={e => setNewNotes(e.target.value)}
              style={{ width: '100%', padding: '6px 10px', marginBottom: 8, background: '#111', border: '1px solid #333', color: '#e0e0e0', borderRadius: 4, boxSizing: 'border-box', fontFamily: 'monospace', fontSize: 13 }}
            />
            <textarea
              ...
              rows={18}
              style={{ width: '100%', padding: '8px 10px', marginBottom: 10, background: '#111', border: '1px solid #333', color: '#e0e0e0', borderRadius: 4, boxSizing: 'border-box', fontFamily: 'monospace', fontSize: 12, resize: 'vertical' }}
            />
            <button
              onClick={handleCreateVersion}
              disabled={!newScript.trim()}
              style={{ padding: '8px 18px', background: newScript.trim() ? '#1a1a3a' : '#111', border: `1px solid ${newScript.trim() ? '#ffd700' : '#333'}`, color: newScript.trim() ? '#ffd700' : '#555', borderRadius: 4, cursor: newScript.trim() ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 600 }}
            >
