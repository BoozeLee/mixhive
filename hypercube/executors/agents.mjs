...
function record(cls, target, dims, verdict, defect, reason) {
  appendCell({ class: cls, target, dims, env: ENV, verdict, oracles: {}, defect: defect ?? null, ...(reason ? { reason } : {}) });
...
// 1. Lua (Lupa) — public health GET.
{
  ...
  record('agent-lua', 'agent:lua', { probe: 'health' }, ok ? 'pass' : 'fail', ok ? null : `status=${r.status} body=${JSON.stringify(r.body)?.slice(0, 80)}`);
}
...
// 2. Wasmoon (strategic runtime) — public health GET.
{
  ...
  record('agent-wasmoon', 'agent:wasmoon', { probe: 'health' }, ok ? 'pass' : 'fail', ok ? null : `status=${r.status}`);
}

// 3+4. Cron dispatch — auth WALL (401 without token) is verifiable now;
// authorized run needs CRON_SECRET.
for (const [cls, path] of [['agent-strategic', '/api/cron/strategic-agents'], ['agent-notif', '/api/cron/notification-prioritizer']]) {
  const noTok = await http(path, { method: 'POST' });
  const wallOk = noTok.status === 401 || noTok.status === 403;
  record(cls, `api:POST ${path}`, { auth: 'anon' }, wallOk ? 'pass' : 'fail', wallOk ? null : `expected 401, got ${noTok.status}`);
  if (CRON) {
    const withTok = await http(path, { method: 'POST', bearer: CRON });
    const runOk = withTok.status === 200;
    record(cls, `api:POST ${path}`, { auth: 'completed' }, runOk ? 'pass' : 'fail', runOk ? null : `authorized run status=${withTok.status}`);
  } else {
...
record('agent-session-spirit', 'agent:session-spirit', { probe: 'cooldown' }, 'blocked', null, 'no-credential');

// 6. Audio worker — offline Go selftest (if toolchain + a sample present).
{
  const hasGo = (() => { try { execSync('go version', { stdio: 'ignore' }); return true; } catch { return false; } })();
  ...
  if (hasGo && sample) {
    try {
      ...
      const out = execFileSync('go', ['run', './worker/audio', '--selftest', sample], { encoding: 'utf8', timeout: 60000 });
      const ok = /selftest ok/i.test(out);
      record('agent-audio', 'agent:audio-worker', { probe: 'selftest' }, ok ? 'pass' : 'fail', ok ? null : out.slice(0, 80));
    } catch (e) {
      record('agent-audio', 'agent:audio-worker', { probe: 'selftest' }, 'fail', String(e).slice(0, 80));
    }
  } else {
    record('agent-audio', 'agent:audio-worker', { probe: 'selftest' }, 'blocked', null, hasGo ? 'no-sample' : 'no-go-toolchain');
  }
}

// 7. AI-Band bridge — provenance publish needs a user JWT.
record('agent-aiband', 'agent:ai-band-bridge', { probe: 'publish' }, 'blocked', null, 'no-credential');
