...
export async function POST(req: NextRequest) {
  ...
  const { data: profile, error: profileError } = await sb
    .from('profiles')
    .select('username, display_name, avatar_url, bio, location, website, genres, social_links, dj_equipment, dj_daw, dj_style')
    .eq('id', ctx.userId)
    .single()
  ...
  const pitchParts = compactList([
    ...
    mixes?.length ? `Featured MIXHIVE catalogue includes ${mixes.length} published mix${mixes.length === 1 ? '' : 'es'}.` : null,
  ])
  ...
  const content = {
    ...
    technical_notes: compactList([
      Array.isArray(profile.dj_equipment) && profile.dj_equipment.length ? `Equipment: ${profile.dj_equipment.join(', ')}` : null,
      Array.isArray(profile.dj_daw) && profile.dj_daw.length ? `DAW/software: ${profile.dj_daw.join(', ')}` : null,
    ]),
