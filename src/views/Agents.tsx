...
const TRIGGER_OPTIONS: { value: LuaAgentTrigger; label: string; description: string }[] = [
  ...
  { value: 'on_unfollow', label: 'On unfollow', description: 'Someone unfollows you.' },
  {
    value: 'on_mix_upload',
    label: 'On mix upload',
    description: 'A DJ you follow publishes a mix.',
  },
  ...
  { value: 'on_repost', label: 'On repost', description: 'Someone reposts your mix.' },
  {
    value: 'on_schedule',
    label: 'On schedule',
    description: 'Runs on a cron schedule you configure.',
  },
...
export function Agents() {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 16px' }}>
      {/* Navigation to Quest experience (Phase 6) */}
      <div style={{ marginBottom: 16 }}>
        ...
        >
          → View My Quests (Mythic Quest Lines prototype)
        </Link>
      </div>
      ...
      >
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: colors.text.muted, fontSize: fontSize.md }}>
            ...
            <a href="/agents/gallery" style={{ color: colors.accent }}>
              Browse the gallery →
            </a>
          </p>
        </div>
          ...
          {agents.map(a => (
            <li key={a.id}>
              <AgentRow
                agent={a}
                onEdit={() => setEditing(a)}
                onChanged={async () => setAgents(await listAgents())}
              />
            </li>
          ))}
        </ul>
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          >
            ...
            <span
              style={{
                color: agent.enabled ? colors.success : colors.text.dim,
          ...
          {TRIGGER_OPTIONS.map(opt => (
            <label
              ...
              style={{
                ...
                border: `1px solid ${trigger === opt.value ? colors.accent : colors.border}`,
                background: trigger === opt.value ? colors.surfaceHover : colors.bg,
              }}
            >
              <input
                type="radio"
                ...
                onChange={() => pickTrigger(opt.value)}
                style={{ marginRight: 6 }}
              />
              <span style={{ fontWeight: fontWeight.semibold }}>{opt.label}</span>
              <div style={{ marginTop: 4, color: colors.text.muted, fontSize: fontSize.xs }}>
                {opt.description}
              </div>
            </label>
          ))}
        ...
        <section>
          ...
          >
            {runs.map(r => (
              ...
              >
                <strong style={{ color: r.status === 'ok' ? colors.success : colors.danger }}>
                  {r.status}
                </strong>{' '}
                · {r.triggered_by} · {r.duration_ms}ms · {new Date(r.created_at).toLocaleString()}
                {r.error_message && (
                  <div style={{ color: colors.danger, marginTop: 4 }}>{r.error_message}</div>
                )}
                {r.stdout && (
                  <pre style={{ marginTop: 4, color: colors.text.dim, whiteSpace: 'pre-wrap' }}>
                    {r.stdout}
                  </pre>
                )}
              </li>
