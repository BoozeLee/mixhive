...
export async function POST(req: NextRequest) {
  ...
  if (!ctx.isPro) {
    return NextResponse.json(
      { error: 'pro_required', message: 'Upgrade to MixHive Pro to use the AI Art Studio.' },
      { status: 403 }
    );
  }
  ...
  const prompt = userPrompt
    ? `Masterpiece, best quality, ultra-detailed, 8k, ${userPrompt}, ${STYLE_SEEDS[styleKey]}, highly detailed textures, intricate details, sharp focus, seamless blend`
    : `${STYLE_SEEDS[styleKey]}, masterpiece, best quality, ultra-detailed, 8k, seamless blend of references`;
  ...
  // Use the highest-weighted reference as the base image for img2img
  const primary = refs.reduce((best, r) => (r.weight > best.weight ? r : best), refs[0]);
  const imgMatch = primary.data.match(/^data:image\/(\w+);base64,(.+)$/);
  if (!imgMatch) {
  ...
  for (let i = 0; i < refs.length; i++) {
    const w = refs[i].weight;
    if (w >= 1.0) {
      promptParts.push(`reference ${i + 1} elements (weight ${w.toFixed(1)})`);
    } else {
      promptParts.push(`reference ${i + 1} hints (weight ${w.toFixed(1)})`);
    }
  }
  ...
  if (contentType.includes('application/json')) {
    ...
    if (json.estimated_time) {
      return NextResponse.json(
        { error: 'Model is loading, please retry', estimated_time: json.estimated_time },
        { status: 503 }
      );
    }
  }
  ...
  return NextResponse.json({ url: dataUrl, prompt: finalPrompt, style: styleKey });
}
