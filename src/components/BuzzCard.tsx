...
import { likeBuzz, unlikeBuzz, hasBuzzLiked, repostBuzz, unrepostBuzz, hasBuzzReposted, deleteBuzz } from '../lib/api'
...
const LANG_LABELS: Record<string, string> = {
  lua: 'Lua', javascript: 'JS', typescript: 'TS', python: 'Python',
  bash: 'Bash', sql: 'SQL', json: 'JSON', rust: 'Rust', go: 'Go',
}
...
export function BuzzCard({ buzz, compact = false, onDeleted }: Props) {
  ...
  const authorName = buzz.author?.display_name || buzz.author?.username || buzz.author_id.slice(0, 8)
  ...
  return (
    <>
      <div
        style={card}
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = colors.borderStrong; initLike(); initRepost() }}
      ...
      >
        ...
        <div style={{ display: 'flex', gap: space[7], marginBottom: space[6] }}>
          ...
          >
            <div style={{
              ...
              fontSize: fontSize.md, fontWeight: fontWeight.bold, color: colors.accent,
            }}>
              {authorAvatar
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: space[3], flexWrap: 'wrap' }}>
              <Link
                to={`/u/${authorUsername}`}
                onClick={e => e.stopPropagation()}
                style={{ color: colors.text.primary, fontWeight: fontWeight.semibold, fontSize: fontSize.base, textDecoration: 'none' }}
              >
                {authorName}
              </Link>
              <span style={{ color: colors.text.muted, fontSize: fontSize.sm }}>@{authorUsername}</span>
              <span style={{ color: colors.text.faint, fontSize: fontSize.sm }}>·</span>
              <span style={{ color: colors.text.dim, fontSize: fontSize.sm }}>{timeAgo(buzz.created_at)}</span>
            </div>
          </div>
        ...
        {buzz.image_url && (
          <button
            type="button"
            style={{ display: 'block', width: '100%', marginBottom: space[6], cursor: 'zoom-in', borderRadius: radius.lg, overflow: 'hidden', padding: 0, border: 'none', background: 'transparent' }}
          ...
          >
            <img
              ...
              alt="Buzz attachment"
              loading="lazy"
              style={{ width: '100%', maxHeight: compact ? 200 : 360, objectFit: 'cover', display: 'block' }}
            />
          </button>
        )}
        ...
        {buzz.audio_url && (
          <div
            style={{ marginBottom: space[6], padding: `${space[5]}px ${space[7]}px`, background: colors.surfaceMuted, borderRadius: radius.lg }}
          >
            <p style={{ margin: `0 0 ${space[3]}px`, fontSize: fontSize.sm, color: colors.text.muted }}>🎵 Audio clip</p>
            {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
            <audio controls style={{ width: '100%', accentColor: colors.accent }} src={buzz.audio_url} />
          </div>
        )}
        ...
        {buzz.video_url && (
          <div
            style={{ marginBottom: space[6], borderRadius: radius.lg, overflow: 'hidden', background: '#000' }}
          >
            {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
            <video
              controls
        ...
        {buzz.code_snippet && (
          <div
            style={{ marginBottom: space[6], borderRadius: radius.lg, overflow: 'hidden', border: `1px solid ${colors.borderStrong}` }}
          >
            {buzz.code_language && (
              <div style={{
        ...
        {buzz.attached_mix && (
          ...
          >
            <div style={{
              display: 'flex', gap: space[7], alignItems: 'center',
              padding: space[7],
              ...
              {buzz.attached_mix.artwork_url && (
                <img
                  ...
                  style={{ width: 48, height: 48, borderRadius: radius.md, objectFit: 'cover', flexShrink: 0 }}
                />
              )}
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: fontSize.sm, fontWeight: fontWeight.semibold, color: colors.text.primary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        {/* Actions */}
        <div
          style={{ display: 'flex', gap: space[2], marginTop: space[4] }}
        >
          {/* Like */}
          <button
      ...
      {imgExpanded && buzz.image_url && (
        ...
        >
          <img src={buzz.image_url} alt="Buzz attachment" style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain' }} />
        </div>
      )}
...
function BodyText({ body }: { body: string }) {
  ...
  return (
    <>
      {parts.map((p, i) =>
        p.type === 'mention' ? (
          <Link key={i} to={`/u/${p.value.slice(1)}`} style={{ color: colors.accent, textDecoration: 'none', fontWeight: 500 }} onClick={e => e.stopPropagation()}>
            {p.value}
          </Link>
        ) : p.type === 'hashtag' ? (
          <Link key={i} to={`/search?q=${encodeURIComponent(p.value)}`} style={{ color: colors.info, textDecoration: 'none' }} onClick={e => e.stopPropagation()}>
            {p.value}
          </Link>
        ) : (
