...
export async function POST(req: NextRequest) {
  ...
  if (profile?.dj_equipment?.length)
    profileLines.push(`Equipment: ${profile.dj_equipment.join(', ')}`);
...
Return only valid JSON. No markdown. No commentary outside the JSON object.`;
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      { error: (err as Record<string, unknown>)?.error ?? 'GPT request failed' },
      { status: res.status }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'GPT returned invalid JSON', raw: rawContent },
      { status: 500 }
    );
  }
  ...
  if (insertErr) {
    return NextResponse.json(
      { error: 'Failed to store suggestion', detail: insertErr.message },
      { status: 500 }
    );
  }
