  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 16px 96px' }}>
      ...
      <section style={{ marginBottom: space[12] }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: colors.text.primary, marginBottom: space[4] }}>
          Trending Mixes
        </h2>
        {loading ? (
          <div style={{ display: 'grid', gap: space[4] }}>
            {[1, 2, 3].map(index => (
              <div key={index} style={{ height: 98, background: colors.surface, borderRadius: radius.lg, border: `1px solid ${colors.border}` }} />
            ))}
          </div>
        ) : mixes.length === 0 ? (
