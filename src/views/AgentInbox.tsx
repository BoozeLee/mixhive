...
function OpportunityCard({ opp }: { opp: Opportunity }) {
  ...
  return (
    <article style={{
      background: 'linear-gradient(135deg, rgba(255,216,74,0.04), transparent 32%), rgba(7,7,5,0.78)',
      border: `1px solid ${colors.accentMuted}`,
      borderRadius: radius.xl,
      padding: `${space[7]}px ${space[8]}px`,
      display: 'flex', flexDirection: 'column', gap: space[4],
    }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: space[5] }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: space[3], marginBottom: space[2], flexWrap: 'wrap' }}>
            <span style={{
          ...
          <h3 style={{ margin: 0, fontSize: fontSize.md, fontWeight: fontWeight.bold, color: colors.text.primary, lineHeight: 1.4 }}>
            {opp.title}
          </h3>
          {opp.organizer && (
            <div style={{ fontSize: fontSize.xs, color: colors.text.muted, marginTop: space[1] }}>by {opp.organizer}</div>
          )}
        </div>
        {daysUntil !== null && (
          <div style={{
            ...
            <div style={{ fontSize: fontSize.xs, color: daysUntil <= 7 ? colors.danger : colors.text.muted }}>Deadline</div>
            <div style={{ fontSize: fontSize.base, fontWeight: fontWeight.bold, color: daysUntil <= 7 ? colors.danger : colors.text.secondary }}>
      ...
      {opp.description && (
        <p style={{ margin: 0, fontSize: fontSize.base, color: colors.text.muted, lineHeight: 1.6, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {opp.description}
        </p>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: space[5], flexWrap: 'wrap' }}>
        {opp.compensation && (
          <span style={{ fontSize: fontSize.xs, color: colors.success }}>💶 {opp.compensation}</span>
        )}
        {opp.genres.slice(0, 3).map(g => (
          <span key={g} style={{
            fontSize: fontSize.xs, color: colors.text.dim,
            padding: `${space[1]}px ${space[3]}px`,
            borderRadius: radius.pill, background: 'rgba(255,255,255,0.04)',
  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{
          width: 22, height: 22, flexShrink: 0, borderRadius: radius.sm,
          border: `2px solid ${colors.accentMuted}`, background: 'transparent',
          cursor: 'pointer', display: 'grid', placeItems: 'center',
          color: colors.accent, fontSize: 12, transition: transition.fast,
        }}
      >
        ✓
      </button>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: fontSize.base, color: colors.text.primary, fontWeight: fontWeight.medium }}>
          {task.title}
        </div>
      ...
      <div style={{
        ...
        background: task.priority === 1 ? colors.danger : task.priority === 2 ? colors.warning : colors.text.faint,
      }} />
    </div>
  )
}
...
export function AgentInbox() {
  ...
  const [loading, setLoading] = useState<Record<Tab, boolean>>({ suggestions: true, tasks: true, opportunities: true })
  const [error, setError] = useState<Record<Tab, string | null>>({ suggestions: null, tasks: null, opportunities: null })
  ...
  const tabs: { key: Tab; label: string; count?: number }[] = [
    { key: 'suggestions', label: 'Suggestions', count: suggestions.filter(s => s.status === 'pending').length || undefined },
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: `${space[9]}px ${space[8]}px` }}>
        ...
        {tabs.map(t => (
          <button
            ...
            style={{
              ...
              padding: `${space[4]}px ${space[6]}px`,
              fontSize: fontSize.base, fontWeight: tab === t.key ? fontWeight.bold : fontWeight.normal,
              color: tab === t.key ? colors.accent : colors.text.muted,
              borderBottom: `2px solid ${tab === t.key ? colors.accent : 'transparent'}`,
              transition: transition.base, marginBottom: -1,
      ...
      {tab === 'suggestions' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: space[6] }}>
          {loading.suggestions && <div style={{ color: colors.text.muted, fontSize: fontSize.base }}>Loading suggestions…</div>}
          {error.suggestions && <div style={{ color: colors.danger, fontSize: fontSize.base }}>{error.suggestions}</div>}
          {!loading.suggestions && suggestions.length === 0 && (
            <div style={{ textAlign: 'center', padding: `${space[12]}px 0`, color: colors.text.muted }}>
      ...
      {tab === 'tasks' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: space[4] }}>
          {loading.tasks && <div style={{ color: colors.text.muted, fontSize: fontSize.base }}>Loading tasks…</div>}
          {error.tasks && <div style={{ color: colors.danger, fontSize: fontSize.base }}>{error.tasks}</div>}
          {!loading.tasks && tasks.length === 0 && (
            <div style={{ textAlign: 'center', padding: `${space[12]}px 0`, color: colors.text.muted }}>
              <div style={{ fontSize: 32, marginBottom: space[5] }}>✓</div>
              <div style={{ fontSize: fontSize.md }}>All caught up.</div>
              <div style={{ fontSize: fontSize.base, marginTop: space[3] }}>No open tasks right now.</div>
            </div>
          )}
          {tasks.map(t => (
      ...
      {tab === 'opportunities' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: space[6] }}>
          {loading.opportunities && <div style={{ color: colors.text.muted, fontSize: fontSize.base }}>Loading opportunities…</div>}
          {error.opportunities && <div style={{ color: colors.danger, fontSize: fontSize.base }}>{error.opportunities}</div>}
          ...
          <p style={{ margin: `${space[6]}px 0 0`, fontSize: fontSize.xs, color: colors.text.faint, textAlign: 'center', lineHeight: 1.6 }}>
            MixHive never contacts anyone automatically. All applications and outreach require your review.
          </p>
        </div>
      )}
    </div>
