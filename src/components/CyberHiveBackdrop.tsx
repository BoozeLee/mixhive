...
export function CyberHiveBackdrop() {
  ...
  useEffect(() => {
    ...
    if (!mount || window.matchMedia('(prefers-reduced-motion: reduce)').matches || !canUseWebGL()) return
    ...
    const hexMaterial = new THREE.LineBasicMaterial({ color: 0xf6c400, transparent: true, opacity: 0.12 })
    const hexes: THREE.Line[] = []
    for (let i = 0; i < 18; i += 1) {
      const hex = new THREE.Line(hexGeometry, hexMaterial)
      hex.position.set((Math.random() - 0.5) * 150, (Math.random() - 0.5) * 84, -25 - Math.random() * 35)
