...
export function useRealtime(userId: string | null, options?: {
  ...
  const triggerMixUpload = async (mixData: any) => {
    try {
      ...
      setState(prev => ({ ...prev, error: error instanceof Error ? error.message : 'Failed to broadcast mix upload' }))
    }
  }
  ...
  const triggerComment = async (commentData: any) => {
    try {
      ...
      setState(prev => ({ ...prev, error: error instanceof Error ? error.message : 'Failed to broadcast comment' }))
    }
  }
  ...
  const triggerNotification = async (notificationData: any) => {
    try {
      ...
      setState(prev => ({ ...prev, error: error instanceof Error ? error.message : 'Failed to broadcast notification' }))
    }
  }
