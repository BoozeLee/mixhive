...
export async function resolveAiContext(req: NextRequest): Promise<AiContext> {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    return { openaiKey: null, isAdmin: false, isPro: false, userId: null, error: 'Not authenticated' }
  }
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    return { openaiKey: null, isAdmin: false, isPro: false, userId: null, error: 'Supabase not configured' }
  }
  ...
  if (authErr || !user) {
    return { openaiKey: null, isAdmin: false, isPro: false, userId: null, error: 'Invalid session' }
  }
  ...
  return Response.json(
    {
      ...
      instructions: 'Add your key in Settings → AI & Creativity, or upgrade to MixHive Pro for built-in AI art generation.',
