...
try {
  ...
  console.log(chalk.yellow('⚠️  Integration tests failed'));
  console.log(chalk.yellow('💡 This may be expected if the database is not set up'));
}

// Coverage tests
try {
  console.log(chalk.blue('📊 Running coverage tests...'));
  execSync('npm run test:coverage', { stdio: 'inherit' });
  console.log(chalk.green('✅ Coverage tests completed'));
} catch (error) {
  console.log(chalk.yellow('⚠️  Coverage tests failed'));
  console.log(chalk.yellow('💡 This may be expected if coverage is not configured'));
}
...
try {
  ...
  execSync('node scripts/deploy-verify.js', { stdio: 'inherit' });
  console.log(chalk.green('✅ Deployment verification passed'));
} catch (error) {
  console.log(chalk.red('❌ Deployment verification failed'));
  console.log(chalk.yellow('💡 Fix the verification issues and try again'));
  process.exit(1);
}
