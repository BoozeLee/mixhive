...
export function EditMix() {
  ...
  async function handleDelete() {
    if (!id) return
    if (!window.confirm('Are you sure you want to delete this mix? This action cannot be undone.')) return
    try {
      await deleteMix(id)
      navigate('/feed')
    } catch {
  ...
  if (loading || !mix) {
    return <div style={{ textAlign: 'center', padding: '40px 20px', color: '#666' }}>Loading...</div>
  }
      ...
      <form onSubmit={e => e.preventDefault()}>
        ...
        <div style={{ marginTop: 24, display: 'flex', gap: 12 }}>
          <button
            ...
            disabled={uploading}
            style={{ flex: 1, background: '#2a1010', color: '#f55', border: 'none', borderRadius: 8, padding: '12px 16px', fontSize: 14, cursor: uploading ? 'not-allowed' : 'pointer', opacity: uploading ? 0.6 : 1 }}
          >
          ...
          <button
            ...
            onClick={handleSave}
            disabled={uploading}
            style={{ flex: 1, background: '#f0c040', color: '#0a0a0a', border: 'none', borderRadius: 8, padding: '12px 16px', fontSize: 14, fontWeight: 600, cursor: uploading ? 'not-allowed' : 'pointer', opacity: uploading ? 0.6 : 1 }}
          >
