 ...
 * broken end to end. Anonymous callers still work and still see only published
 * events.
 */
      ...
      try {
        ...
        if (!cancelled) {
          if (res.ok) {
            ...
            // leave event as null so not-found state renders
          } else {
            setError(`Failed to load event (${res.status})`);
          }
        }
      } catch (err) {
