...
export function SearchAutocomplete({
  ...
  const visibleSuggestions = query.trim() ? suggestions : getPopularSearches().slice(0, 6).map(title => ({
    type: 'genre' as const,
    title,
          ...
          {visibleSuggestions.map((suggestion, index) => (
            <button
              ...
              style={{
                ...
                border: 0,
                borderBottom: index === visibleSuggestions.length - 1 ? 0 : `1px solid ${colors.border}`,
                color: colors.text.primary,
                cursor: 'pointer',
                textAlign: 'left',
              }}
            >
              <span aria-hidden="true">{suggestion.type === 'profile' ? '@' : suggestion.type === 'mix' ? '♪' : '#'}</span>
              <span style={{ flex: 1, minWidth: 0 }}>
                <span style={{ display: 'block', fontSize: 14, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {suggestion.title}
                </span>
                {suggestion.subtitle && (
                  <span style={{ display: 'block', color: colors.text.dim, fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {suggestion.subtitle}
                  </span>
                )}
              </span>
